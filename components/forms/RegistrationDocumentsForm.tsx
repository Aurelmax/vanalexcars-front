...
import { formService, RegistrationDocumentsFormData, RegistrationDocumentsFormValues } from '../../lib/services/formService';
...
const RegistrationDocumentsForm: React.FC<RegistrationDocumentsFormProps> = ({
  ...
  const { values, errors, getFieldProps, setFieldValue, handleSubmit, reset } =
    useForm<RegistrationDocumentsFormValues>({
      initialValues: {
        name: '',
        email: '',
        phone: '',
      ...
      validate: values => {
        ...
        if (!(values.name as string)?.trim()) {
          errors.name = 'Le nom est requis';
        }

        if (!(values.email as string)?.trim()) {
      ...
      onSubmit: async values => {
        ...
        try {
          ...
          formData.append('request_type', values.request_type as string);
          formData.append('urgency', values.urgency as string);
          formData.append('message', (values.message as string) || '');

          // Ajouter les fichiers
          const documents = values.documents as File[];
    ...
    <div className='max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8'>
      ...
      <form onSubmit={handleSubmit} className='space-y-8'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            ...
            <div>
              ...
              <select
                id='urgency'
                name='urgency'
                value={values.urgency as string}
        ...
        <div className='bg-yellow-50 border border-yellow-200 rounded-lg p-6'>
          ...
          {((values.documents as File[])?.length > 0) && (
                ...
                <div className='bg-white rounded-lg border border-green-200 p-3'>
                    ...
                    <div className='ml-auto'>
                      {(values.documents as File[])?.length > 0 ? (
                        <span className='text-green-600 text-sm'>
                          ✓ {(values.documents as File[]).length} fichier(s)
                        </span>
                      ) : (
                  ...
                  {(values.documents as File[])?.length > 0 && (
                    <div className='text-xs text-gray-600'>
                      {(values.documents as File[]).map((file, index) => (
                ...
                <div className='bg-white rounded-lg border border-green-200 p-3'>
                  <div className='flex items-center mb-2'>
                    ...
                    <div className='ml-auto'>
                      {(values.documents as File[])?.length > 0 ? (
                        <span className='text-green-600 text-sm'>
                          ✓ {(values.documents as File[]).length}{' '}
                          fichier(s)
                        </span>
                      ) : (
                        <span className='text-gray-400 text-sm'>
